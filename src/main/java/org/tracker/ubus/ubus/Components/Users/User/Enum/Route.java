...
public enum Route {
    ...
    ROUTE_1("DFC-APK",
            List.of(DFC, APB, APK, APB, DFC), false),
    ROUTE_2("SWC-APB",
            List.of(SWC, APK, APB, APK, SWC), false),

    ROUTE_3("SWC-DFC",
            List.of(SWC, DFC), true),

    ROUTE_JBS("APK-JBS",
              List.of(APK, APB, JBS, APB, APK), false);
    ...
    public static Collection<Route> findRouteByDestinations(Collection<Destination> destinations) throws IllegalArgumentException{
        Predicate<Route> isContained = route -> route.getDestinations().containsAll(destinations);
    ...
    public static BusPrefView[] getAllValidDestinationCombinations() {
        var straightLineRoutes = List.of(ROUTE_1, ROUTE_2, ROUTE_JBS);

        return straightLineRoutes.stream()
                .flatMap(route -> {
                    var destinations = route.getDestinations();
                    var uniqueDestinations = destinations.stream()
                            .distinct()
                            .toList();
                    var combinations = new ArrayList<BusPrefView>();
                    for (int i = 0; i < uniqueDestinations.size(); i++) {
                        for (int j = i + 1; j < uniqueDestinations.size(); j++) {
                            combinations.add(new BusPrefView(uniqueDestinations.get(i), uniqueDestinations.get(j)));
                        }
                    }
                    return combinations.stream();
                })
                .distinct()
                .toArray(BusPrefView[]::new);
    }
